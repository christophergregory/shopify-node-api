...
ShopifyAPI.prototype.buildAuthURL = function() {
    var parsedUrl = url.parse(this.config.shop);
    // Since shop can be a slug and also an url let's handle if slug given
    if (!parsedUrl.host) parsedUrl.host = this.config.shop + '.myshopify.com'

    var auth_url = 'https://' + parsedUrl.host;
